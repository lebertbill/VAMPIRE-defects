...
namespace create{
   ...
   void cluster_vacancy(std::vector<cs::catom_t> &);
...
int create_system_type(std::vector<cs::catom_t> & catom_array){
   ...
   // if (mp::material[0].defect==true){
   //    cluster_vacancy(catom_array);
   // }
...
//creates cluster vacancies randomly
void cluster_vacancy(std::vector<cs::catom_t> & catom_array){
   if (mp::material[0].defect==true){
      
      int cluster_vac_percentage=mp::material[0].cluster_vacancy_percentage;  //cluster vacancy percentage
      int num_vacancy_in_cluster=vin::read_material[0].num_vacancy_per_cluster;   //no of vacancy per cluster
      int total_missing_atoms=(catom_array.size()/100)*cluster_vac_percentage;
      int total_clusters=total_missing_atoms/num_vacancy_in_cluster;   //total number of cluster for given vacancy percentage
      std::cout << "Number of vacancies per cluster:  " << num_vacancy_in_cluster << std::endl;
      std::cout << "Cluster vacancy percentage: " << cluster_vac_percentage << std::endl;
      zlog<< zTs() << "Number of vacancies per cluster:  " << num_vacancy_in_cluster << std::endl;
      zlog<< zTs()<< "Cluster vacancy percentage: " << cluster_vac_percentage << std::endl;
      std::cout << "Total number of clusters in the system (per processor in case of mpi): " << total_clusters << std::endl;
      zlog<< zTs()<< "Total number of clusters in the system (per processor in case of mpi): " << total_clusters << std::endl;      
      std::cout << "Number of atoms before creating vacancies: " << catom_array.size() << std::endl;
      zlog<< zTs()<< "Number of atoms before creating vacancies: " << catom_array.size() << std::endl;

      int min=20;
      int max=catom_array.size()-20;
      for (int i=0;i<total_clusters;i++){
         srand(time(0));
         int cluster_core=rand()% (max-min)+min;
         for (int j=0;j<num_vacancy_in_cluster;j++){
            catom_array.erase(catom_array.begin()+cluster_core+j+rand()%4);
            }
      }create::internal::calculate_atomic_composition(catom_array);
 ...
 //  for (int i=1000;i<1250;i++){
  //    catom_array.erase(catom_array.begin()+i);
   //}
  // for (int i=8000;i<8250;i++){
   //   catom_array.erase(catom_array.begin()+i);
  // }
