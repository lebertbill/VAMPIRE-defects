...
namespace vin{
                ...
                std::string const filename_string)
    {
            ...
            test="cluster-vacancy-percentage";
            if(word==test){
                unsigned int vac_percentage = atoi(value.c_str());
                check_for_valid_int(vac_percentage, word, line, prefix, 1, 100,"material","1 - 100");
                
                read_material[super_index].defect=true;
                read_material[super_index].cluster_vacancy_percentage=vac_percentage;
                return EXIT_SUCCESS;
            }
            //---------------------------------------------------------------
            else
            test="num-vacancy-per-cluster";
            if(word==test){
                unsigned int per_cluster = atoi(value.c_str());
                check_for_valid_int(per_cluster, word, line, prefix, 1, 100,"material","1 - 100");
               // 
                read_material[super_index].num_vacancy_per_cluster=per_cluster;
